...
// Alphabet without confusable characters (0/O, 1/l/I, etc.)
const ALPHABET = 'ABCDEFGHJKLMNPQRSTUVWXYZabcdefghjkmnpqrstuvwxyz23456789';

/**
 * Generate a random 4-character password using cryptographic randomness.
 * Avoids easily-confused characters so techs can read it off a phone screen.
 */
export function generate4CharPassword(): string {
...
export async function sendOnboardingEmail(opts: {
  ...
  const subject = 'TBC ინვენტარიზაცია — თქვენი წვდომა';
  ...
  const html = `
    <div style="font-family:system-ui,-apple-system,sans-serif;max-width:480px;padding:20px;line-height:1.55;color:#0f172a">
      ...
      <p>TBC × DMT ფილიალების ინვენტარიზაციის სისტემაში შენთვის ანგარიში შექმნილია.</p>
      ...
      <table style="width:100%;border-collapse:collapse;margin:18px 0;background:#F1F5F9;border-radius:8px;overflow:hidden;">
        ...
        <tr><td style="padding:10px 14px;font-size:12px;color:#64748B;text-transform:uppercase;letter-spacing:0.05em;">დროებითი პაროლი</td>
            <td style="padding:10px 14px;font-family:ui-monospace,monospace;font-weight:700;font-size:22px;color:#00AA8D;letter-spacing:0.15em;">${escapeHtml(opts.tempPassword)}</td></tr>
      </table>

      <p style="text-align:center;margin:24px 0">
        <a href="${opts.loginUrl}" style="background:#0071CE;color:#fff;padding:12px 22px;border-radius:8px;text-decoration:none;font-weight:600;display:inline-block">შესვლა</a>
      </p>
      ...
      <p style="color:#64748B;font-size:12px;margin-top:18px">
        შესვლისთანავე დაეთანხმე <a href="${opts.loginUrl.replace('/tbc', '/tbc/nda')}" style="color:#0071CE">NDA წესებს</a>.
      </p>
      <p style="color:#94a3b8;font-size:11px;margin-top:14px">— TBC × DMT ინვენტარიზაცია · engineers.ge</p>
    </div>`;

  if (!key) {
    console.log('[tbc onboarding] RESEND_API_KEY missing — would send:', {
      to: opts.to,
  ...
  if (!res.ok) {
    ...
    console.error('[tbc onboarding] resend failed', res.status, txt);
