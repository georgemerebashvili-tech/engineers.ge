...
const COOKIE_NAME = 'construction_jwt';
const COOKIE_MAX_AGE = 60 * 60 * 24 * 30; // 30 days

export type ConstructionSession = {
  uid: string;
  username: string;
  role: 'admin' | 'user';
...
type ConstructionResponsibleSite = {
  dmt_manager?: string | null;
  kaya_manager?: string | null;
};

function secret() {
  const s = process.env.CONSTRUCTION_JWT_SECRET || process.env.AUTH_SECRET;
  if (!s) throw new Error('CONSTRUCTION_JWT_SECRET (or AUTH_SECRET) is not set');
  return new TextEncoder().encode(s);
}

async function ensureStaticAdmin(username: string, password: string) {
  const envMap: Record<string, string | undefined> = {
    admin: process.env.CONSTRUCTION_ADMIN_PASS_HASH,
    admin_kaya: process.env.CONSTRUCTION_ADMIN_KAYA_PASS_HASH
  };
  const displayMap: Record<string, string> = {
    admin: 'Admin',
    admin_kaya: 'Kaya (Admin)'
  };
...
export async function verifyConstructionLogin(username: string, password: string) {
...
export async function issueConstructionSession(u: {
  id: string;
  username: string;
  role: 'admin' | 'user';
  displayName: string | null;
...
export async function destroyConstructionSession() {
  const store = await cookies();
  store.delete(COOKIE_NAME);
}

export async function getConstructionSession(): Promise<ConstructionSession | null> {
  const store = await cookies();
...
export async function logConstructionLoginEvent(opts: {
  username: string;
  userId?: string | null;
  success: boolean;
}) {
  try {
    ...
    const db = supabaseAdmin();
    await db.from('construction_login_events').insert({
    ...
    console.error('[construction] log login event failed', e);
  }
}

export async function requireConstructionSession(): Promise<ConstructionSession> {
  const s = await getConstructionSession();
  if (!s) throw new Error('unauthorized');
  return s;
}

export async function requireConstructionAdmin(): Promise<ConstructionSession> {
  const s = await requireConstructionSession();
...
function getSessionIdentityAliases(session: ConstructionSession) {
...
export function siteMatchesConstructionSession(
  site: ConstructionResponsibleSite,
  session: ConstructionSession
) {
  const aliases = getSessionIdentityAliases(session);
  if (aliases.length === 0) return false;
  return (
    matchesIdentity(site.dmt_manager, aliases) ||
    matchesIdentity(site.kaya_manager, aliases)
  );
}

export async function getConstructionSiteAccess(
  ...
  const perms = await db
    .from('construction_site_permissions')
    .select('site_id')
  ...
  for (const row of perms.data || []) {
    if (row.site_id == null) seeAll = true;
    else allowedIds.add(row.site_id as number);
  }

  return {seeAll, allowedIds};
}

export async function canAccessConstructionSite(
  db: ReturnType<typeof supabaseAdmin>,
  session: ConstructionSession,
  siteId: number
) {
  const {seeAll, allowedIds} = await getConstructionSiteAccess(db, session);
  if (seeAll || allowedIds.has(siteId)) return true;

  const site = await db
    .from('construction_sites')
    .select('id, dmt_manager, kaya_manager')
    .eq('id', siteId)
    .maybeSingle<ConstructionResponsibleSite & {id: number}>();

  if (!site.data) return false;
  return siteMatchesConstructionSession(site.data, session);
}
