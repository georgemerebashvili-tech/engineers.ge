...
export type ConstructionResetIssueInput = {
  userId: string;
  email: string;
  ip?: string | null;
...
export async function issueConstructionResetToken(opts: ConstructionResetIssueInput) {
  const token = crypto.randomBytes(TOKEN_BYTES).toString('base64url');
  const expires_at = new Date(
    Date.now() + EXPIRES_HOURS * 3600_000
  ).toISOString();
  const {error} = await supabaseAdmin().from('construction_password_reset_tokens').insert({
    token,
    user_id: opts.userId,
...
export type ConstructionResetConsumeResult =
  | {ok: true; userId: string; username: string}
  | {ok: false; reason: 'not_found' | 'already_used' | 'expired' | 'db'};

export async function consumeConstructionResetToken(
  token: string,
  newPassword: string
): Promise<ConstructionResetConsumeResult> {
  const db = supabaseAdmin();
  const {data, error} = await db
    .from('construction_password_reset_tokens')
...
export async function sendConstructionResetEmail(opts: {
  to: string;
  username: string;
  ...
  const subject = 'KAYA Construction — პაროლის აღდგენა';
  ...
  if (!key) {
    console.log('[construction password-reset] RESEND_API_KEY missing — would send:', {
      to: opts.to,
      subject,
  ...
  if (!res.ok) {
    ...
    console.error('[construction password-reset] resend failed', res.status, txt);
