...
- [ ] 🟡 **Calc iframes: per-calc translation tables** · Infrastructure score: **15/15 calcs load `_i18n.js`** (2026-04-21). Translation tables: 4/15 — `hvac` (ka/en/ru/tr/az/hy), `heat-loss`, `wall-thermal`, `stair-pressurization` (ka/en/ru pilot, 2026-04-21). დარჩენილია 11 calc-ის top UI chrome-ის translation: elevator-shaft-press, parking-ventilation, floor-pressurization, wall-editor, building-composer, silencer, silencer-kaya, ahu-ashrae, heat-transfer, ifc-viewer, stair-pressurization-mockup. პატერნი: `engCalcI18n.apply({ka,en,ru,...})` block HTML-ის ბოლოში (pattern: `hvac.html:1587` ან `stair-pressurization.html:4259`).
