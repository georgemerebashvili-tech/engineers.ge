...
- [x] 2026-05-07 — Loads (Qs/Ql/Qh) ამოღებულია Step1 input-ებიდან · ეს იყო ცირკულარული input (user აყენებდა Qs-ს → supplyT-ს ანგარიშობდა → ისევ Qs-ს იწერდა). ახლა supply state default ΔT=12°C/90% RH, chain-ის pipeline-ი override-ს უკეთებს. StepSizing/StepSummary/StepReport — ყველა heating/cooling display ახლა `chain.totalCooling/totalHeating`-დან მოდის. `state.loads` field-ი deprecated optional types-ში (legacy localStorage rows-ისთვის). 6 ფაილი: Step1Inputs, AhuAshrae, types, StepSizing, StepSummary, StepReport.
- [x] 2026-05-07 — StepComponents (Step 3) viewer enhancement · მარცხენა "სექციათა ჯაჭვი" panel ვიწრო (`minmax(360px,440px)_1fr`); 4-button view toolbar (3D / გვერდი / წინა / ზემო); ortho ხედები — pure SVG technical drawing `AhuOrthoSchematic.tsx` (პერსპექტივა WebGL-ით რჩება). Side/Top: per-section dim chain (mm) + total length dim. Front: depth + height dims. ყველა ortho-ში title block: L × H × D mm + სექციების რაოდ. + სრული წონა (kg) — `SECTION_VISUALS[type].weightKg` + `CASING_KG_PER_M = 80`.
- [x] 2026-05-07 — StepComponents drag-and-drop reorder + ASHRAE-compliant ordering rules · ახალი `lib/ahu-ashrae/section-order-rules.ts` — `validateOrder()` + `tryReorder()`; ფილტრები G4→M5→F7→F9→H13/H14 (rank) მკაცრად coarse→fine; pair-rules: damper→filter/HR/mix, HR→mix, mix→coils, preheat→cool, cool→reheat, filter→hum, fan→hum, cool→hum, fan→silencer. UI: HTML5 native DnD `GripVertical` handle + drop indicator line; აკრძალულ drop-ზე transient toast ქართული ახსნით; persistent yellow banner ჯაჭვის violations-ით; collapsible "AHU თანმიმდევრობის წესები" legend.
- [x] 2026-05-07 — StepComponents layout compact · მარცხენა section list panel შევკუმშე (`minmax(280px,340px)`); მარჯვნივ ვერტიკალურად 2 უჯრა — ზევით guard always-on side ortho ხედი (`AhuOrthoSchematic view="side"`), ქვევით 3D პერსპექტივა (`AhuStlViewer`); ორივე დინამიური, კომპონენტთა ცვლილებაზე live განახლდება; view toolbar ამოვშალე (აღარაა საჭირო).
