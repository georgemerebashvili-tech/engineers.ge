...
- [x] 2026-05-04 — ახალი `/dmt/contacts` გვერდი — სრული საკონტაქტო რეესტრი + "→ ლიდად გადაყვანის" ღილაკი (კომპანიის გარეშე disabled). Conversion-ის შემდეგ contact-ზე badge `→ L-NNNN`. Task: [docs/tasks/033-dmt-contacts.md](./tasks/033-dmt-contacts.md). Codex code ✅ 2026-05-04 — build/typecheck OK; DB blocker resolved locally 2026-05-07 — `db:status` confirms `0062_dmt_contacts.sql` applied.

## 🟡 Leads workflow refactor — 3 statuses + labels

- [x] 2026-05-04 — ლიდები: 3 status (`offer_in_progress` → `offer_accepted` / `offer_rejected`), workflow sub-steps (ინვენტარი + ინვოისი), free-form labels autocomplete-ით. Task: [docs/tasks/034-leads-status-workflow.md](./tasks/034-leads-status-workflow.md). Codex code ✅ 2026-05-04 — migration/API/store/components + `/dmt/leads` wired, build/typecheck OK; DB blocker resolved locally 2026-05-07 — `db:status` confirms `0063_leads_status_workflow.sql` applied.
...
- [x] 2026-05-05 — `dmt_offers` ცხრილი + ოფერის შექმნის UI (inventory items + custom) + send to client (share_token) · per-lead photo upload + AI analyze (Anthropic Claude Haiku 4.5 vision) · mobile route `/dmt/m/leads/[id]/photos` · QR code deep-link from desktop. **STRICTLY /dmt scope, NO other modifications. NO DELETIONS.** Task: [docs/tasks/037-dmt-offer-and-lead-inventory.md](./tasks/037-dmt-offer-and-lead-inventory.md). Codex code ✅ 2026-05-05 — offers/photos migrations, APIs, desktop drawer UI, mobile photo flow, QR deep-link, and AI analyze stub shipped; typecheck/lint/build OK; DB blocker resolved locally 2026-05-07 — `db:status` confirms `0067_dmt_offers.sql` and `0068_dmt_lead_inventory_photos.sql` applied.

## 🟡 DMT Offer PDF generation (კომერციული წინადადება)

- [x] 2026-05-05 — PDF generation Task 037-ის ოფერებისთვის. Template matches existing "კომერციული წინადადება" layout (DMT logo, header, items table, totals with margin %, signature block, Sakpatenti footer). Yellow-highlighted fields = dynamic; rest = fixed template. Sequential doc number sequence (219...). pdf-lib + Georgian font. AI optional (skipped in v1). **STRICTLY /dmt scope. NO DELETIONS.** Task: [docs/tasks/038-dmt-offer-pdf-generation.md](./tasks/038-dmt-offer-pdf-generation.md). Codex code ✅ 2026-05-05 — PDF migration/API/renderer/UI shipped; typecheck/lint/build OK; DB blocker resolved locally 2026-05-07 — `db:status` confirms `0069_dmt_offer_pdf_meta.sql` applied.

## 🟡 DMT Offer PDF — template fidelity fixes

- [x] 2026-05-05 — Generated PDF ვიზუალურად ბევრად განსხვავდება ორიგინალისგან. Fix list: logo placement (centered, no overlap), metadata table style, items table 8 columns (with separate ხელობა / labor total), totals format, money-back yellow highlight, exact 13-point HVAC feature list (hardcoded), Sakpatenti emblem on every page, signature page (omit "ს/კ" if empty), continuation page headers. Task: [docs/tasks/039-offer-pdf-template-fidelity.md](./tasks/039-offer-pdf-template-fidelity.md). Codex code ✅ 2026-05-07 — PDF template fidelity pass shipped: centered logo asset, bordered metadata table with yellow doc highlights, 8-column labor table uses labor total, VAT note is informational, guarantee paragraph is highlighted, continuation headers/footer emblem/signature tax handling updated; typecheck/lint/build OK.

## 🟢 OfferEditor — hide description / SKU sub-text

- [x] 2026-05-07 — Items table row-ში description sub-input + source badge ("🏷 SKU" / "✏ custom") UI-დან წაიშალოს. ცხადია მხოლოდ name. **Data preserved** — fields stay in OfferItem type/DB. Task: [docs/tasks/040-offer-editor-hide-description.md](./tasks/040-offer-editor-hide-description.md). Codex code ✅ 2026-05-07 — item rows now show only name; description/sku/source data remains preserved in item objects; typecheck/lint OK.

## 🟡 OfferEditor — yellow-highlighted PDF fields editable

- [x] 2026-05-07 — ორიგინალ PDF-ში ყვითლად მონიშნული ყველა ველი (doc number, doc date, client company, **tax ID**, director name, phone) editable გავხადოთ offer editor-ში. Migration `0070` adds 6 nullable columns to `dmt_offers`. PDF generation uses override values when set, falls back to lead/auto when null. **NO DELETIONS**. Task: [docs/tasks/041-offer-editable-yellow-fields.md](./tasks/041-offer-editable-yellow-fields.md). Codex code ✅ 2026-05-07 — migration/API/types/editor/PDF overrides shipped; typecheck/lint/build OK; DB blocker resolved locally 2026-05-07 — `db:status` confirms `0070_dmt_offers_client_fields.sql` applied.

## 🟡 OfferEditor — editable monthly subscription + form validation

- [x] 2026-05-07 — Add `monthly_subscription` column to `dmt_offers` (nullable). Editable in `3 · ჯამი` section (placeholder 150). PDF uses offer's value or fallback. **Validation everywhere:** items table (name required, qty>0, prices≥0), client tax ID 9-digit format, VAT/margin 0-100%, save/send disabled with errors. Inline red-border highlights + summary banner. **NO DELETIONS**. Task: [docs/tasks/042-offer-subscription-and-validation.md](./tasks/042-offer-subscription-and-validation.md). Codex code ✅ 2026-05-07 — migration/API/types/editor/PDF subscription + validation shipped; typecheck/lint/build OK; DB blocker resolved locally 2026-05-07 — `db:status` confirms `0071_dmt_offers_subscription.sql` applied.

## 🟡 PDF — match test.pdf typography exactly + remove ALL yellow highlights

- [x] 2026-05-07 — Generated PDF ვიზუალურად ემთხვევა test.pdf-ს (typography hierarchy, bold headings, dark text). **ყვითელი სრულად მოშორდეს**: metadata cell highlight, money-back paragraph background, signature page accents, items rows. ყვითელი = template hint = generated PDF-ში არ არის საჭირო (data უკვე filled). Color palette cleaned. Task: [docs/tasks/043-pdf-match-original-no-yellow.md](./tasks/043-pdf-match-original-no-yellow.md). Codex code ✅ 2026-05-07 — PDF yellow purge + cleaner typography palette shipped; metadata/guarantee/footer placeholder no longer use yellow; typecheck/lint/build OK.

## 🟢 OfferEditor runtime ReferenceError + sazeo logo + migrations — DONE

- [x] 2026-05-07 — `/dmt/invoices` PDF generation pipeline fully restored. Task: [docs/tasks/045-offer-editor-runtime-fix-and-logo.md](./tasks/045-offer-editor-runtime-fix-and-logo.md).
  - Codex ✅ 2026-05-07 — `hasClientErrors` audit clean, stale `.next` rebuilt, `sharp` + `npm run crop-logo` + LOGO_SCALE/LOGO_TOP_GAP adjusted, `docs/migrations-pending-bundle.sql` generated.
  - Claude ✅ 2026-05-07 — found user is on **local Supabase** (`127.0.0.1:54321/54322`, default password `postgres`), not blocked cloud — ran `npm run db:migrate` directly → **all 69 migrations applied** including 0067/0069/0070/0071 (`monthly_subscription`, `dmt_next_offer_doc_number()` RPC verified returning 223, `dmt-offers-pdfs` storage bucket present); generated `public/dmt/logo.png` programmatically via `scripts/generate-sazeo-logo.mjs` (sharp SVG render → auto-crop, 1346×418px); fixed bonus issues from screenshot — `[object Object]` error display (`describeApiError` helper in fetchJson + smarter `jsonError` server-side), trial-period number-input typing UX (string buffer + onBlur clamp), VAT default ON 18% / monthly_subscription default 150 / docDate default today / trial-period required validation. Typecheck clean.

## 🟡 Offer — optional sale/discount % field

- [x] 2026-05-07 — User-მა ცხადად ცნო რომ ზოგჯერ "სეილი" აქვს. დაემატოს optional **`discount_percent`** field offer-ზე (0-100% range, nullable). Migration `0072_dmt_offers_discount.sql`. UI: OfferEditor Section 3 · ჯამი → ახალი "ფასდაკლება %" input. Calculation: applied AFTER margin+VAT (final reduction). Summary row visible (-X GEL, green) მხოლოდ თუ value > 0. PDF: extra row breakdown table-ში + page 1 summary paragraph mentions both pre/post discount. Existing offers (null discount) render identically — NO regression. Task: [docs/tasks/048-offer-discount-sale.md](./tasks/048-offer-discount-sale.md). **Codex**, NO DELETIONS, STRICTLY DMT offers scope. Codex code ✅ 2026-05-07 — migration 0072 applied locally, API/store/mapper/editor/PDF wired for optional discount percent, post-discount totals and green summary row shipped; typecheck/lint/build OK.

## 🟡 DMT leads / negotiations — full filters + sort + ActorChip parity with invoices

- [x] 2026-05-07 — User: "მოლაპარაკებების გვერდზე გამართე ფილტრები, სტატუსებით, თარიღით სორტი და ა.შ. ყველა ვარიანტი გამოიყენე". Mirror Task 046 invoices Phase 1+2 pattern on `/dmt/leads`: (1) **სტატუსით** multi-select pills (offer_in_progress / offer_accepted / offer_rejected; OR logic; count badges), (2) **თარიღით** range picker (from/to + 4 presets), (3) **სორტირება** clickable headers — date / status / value (asc/desc + ↑↓), (4) **owner filter** verify existing icon works + dropdown of distinct owners, (5) **labels** multi-select, (6) **"ფილტრი"** header toggle + active count badge, (7) **"ფილტრების გასუფთავება (N)"** link, (8) localStorage persist `dmt:leads:filter`, (9) filtered-vs-empty-DB empty states, (10) **ActorChip** cells for createdBy/updatedBy (Task 053 follow-through). NO DELETIONS, STRICTLY `/dmt/leads/page.tsx`. Task: [docs/tasks/054-leads-filters-and-sort.md](./tasks/054-leads-filters-and-sort.md). Codex ✅ 2026-05-07 — advanced filters/sort + `dmt:leads:filter` persistence + ActorChip cells shipped; typecheck/scoped lint OK.

## 🟡 DMT — actor cells show literal "მე" instead of logged-in user

- [x] 2026-05-07 — User: "კონტაქტებში და მოლაპარაკებებში რატო წერია 'მე'? მე მინდა იყოს ექაუნთის მონაცემები". Root cause: [`lib/dmt/leads-store.ts:173`](./../lib/dmt/leads-store.ts#L173) `getActor()` ცოცხლად აბრუნებს ლიტერალ `'მე'`-ს localStorage-ი ცარიელია, client-supplied `createdBy/updatedBy` server-ზე გადადის და `shared-state-server.ts` `leadToDb()`/`contactToDb()` არ overrides them. Server ტრუსტი არ უნდა იყოს client-ზე. Fix: server routes always use `dmtActor(auth.me)` for `created_by` (insert) + `updated_by` (insert+patch); client-side `getActor()` returns `''`; UI გამოაჩინოს `<ActorChip name={row.createdBy} />` initial-ით + display name. Existing literal `'მე'` rows უცვლელად რჩება — backfill არა-scope. Task: [docs/tasks/053-dmt-actor-display-from-account.md](./tasks/053-dmt-actor-display-from-account.md). Codex ✅ 2026-05-07 — client fallback removed, DMT lead/contact/manual-lead write paths use server actor, shared ActorChip added; typecheck/scoped lint OK.

## 🟡 DMT contacts — per-field validation (phone / email / name / etc.)

- [x] 2026-05-07 — User: "კონტაქტების გვერდზე ვალიდაციები დაადო ტელეფონის ნომერზე რომ ასოები არ ჩაიწეროს, ასევე სხვა თუ რაიმე მსგავსი იქნება". Inline soft validators-ის დამატება: **phone** (`/^[+\d\s\-()]+$/`, min 5 digits), **email** (basic shape), **name** (≤200 chars), **company** (≤200), **position** (≤120), **notes** (≤2000). Visual: red text + bg-red-lt + tooltip title-ით when invalid. Soft-block: ცოცხალი save+persist რჩება — server permissive. STRICTLY `/dmt/contacts` scope. Task: [docs/tasks/052-dmt-contacts-field-validation.md](./tasks/052-dmt-contacts-field-validation.md). Codex ✅ 2026-05-07 — validators wired in `app/dmt/contacts/page.tsx`; typecheck/scoped lint OK.

## 🟡 Offer — subscription discount toggle + manual regular price

- [x] 2026-05-07 — User: "სააბონენტოს გადასახადს ვუკეთებთ თუ არა ფასდაკლებას, ხელით უნდა შეიყვანოს". დაემატოს optional **`subscription_regular_price`** field offer-ზე (numeric 12,2, nullable). Migration `0074_dmt_offers_subscription_discount.sql`. UI: OfferEditor Section 3 · ჯამი → checkbox "სააბონენტოზე ფასდაკლება" + collapsible "წინა (სტანდარტული) ფასი" input. Default 200 toggled-on. PDF: `ნაცვლად N ლარისა` clause appears **მხოლოდ** when `subscription_regular_price` set AND > monthly_subscription. Hardcoded `STANDARD_SUBSCRIPTION = 200` removed from offer-template (currently auto-shows when `< 200`). Existing offers (null) render unchanged. Task: [docs/tasks/051-offer-subscription-discount-toggle.md](./tasks/051-offer-subscription-discount-toggle.md). **Codex**, NO DELETIONS, STRICTLY DMT offers scope. Codex code ✅ 2026-05-07 — migration 0074 applied locally; store/API/editor/PDF wired for explicit subscription regular price; PDF no longer hardcodes 200; typecheck/scoped lint/build OK.

## 🟡 Offer — optional manual margin AMOUNT override (GEL)

- [x] 2026-05-07 — User: "კომერციული მოგებაც ხელით უნდა შევიყვანოთ". დაემატოს optional **`margin_amount_override`** field offer-ზე (numeric 12,2, nullable). Migration `0073_dmt_offers_margin_override.sql`. UI: OfferEditor Section 3 · ჯამი → margin row ახლა აქვს BOTH `%` input AND `₾` absolute override side-by-side. Override set → % disabled, absolute amount used directly. Override null → existing behavior (computed from %). Effective % back-calc-ხდება display-ისთვის. Existing offers (null override) render identically — NO regression. Claude already changed default `marginPercent: 15 → 0` for new offers. Task: [docs/tasks/050-offer-manual-margin-amount.md](./tasks/050-offer-manual-margin-amount.md). **Codex**, NO DELETIONS, STRICTLY DMT offers scope. Codex code ✅ 2026-05-07 — migration 0073 applied locally; store/API/editor/PDF totals honor optional manual margin amount override; typecheck and scoped DMT lint OK.

## 🟡 PDF — match reference (აურა ფიტნესი.pdf) typography 1:1

- [x] 2026-05-07 — Generated PDF visually matches reference sample (აურა ფიტნესი.pdf — 3-page commercial offer): page 1 sazeo wordmark only, metadata `.docx` filename, docId DD.MM.YYYY format (no offer.id suffix), provider "შპს ციფრული მართვის ტექნოლოგიები" restored, navy/blue palette, page 2 intro paragraph reference text, summary inline `(ფასდაკლება N% - X₾)`, ₾ symbol throughout, subscription "ნაცვლად 200 ლარისა" optional clause, full-bold summary + subscription lines, item #7 `ავტომატიზაციას` keyword, signature page values bold, footer copyright bold prefix `საავტორო უფლებები საქპატენტი:` + `დეპონირების რეგისტრაციის #8973/#8993` bold, page 2+ top-right wordmark, sakpatenti emblem extracted from reference test.pdf via `scripts/extract-sakpatenti.mjs`. Claude shipped 2026-05-07; Codex verifies + remaining gaps (items table column widths, totals row rule, page-num font, VAT informational line position). Task: [docs/tasks/049-pdf-match-reference.md](./tasks/049-pdf-match-reference.md). Codex code ✅ 2026-05-07 — PDF-only backlog closed: wider item name columns, no item description sub-text, stronger totals rule, smaller VAT informational line under totals, Georgian-only page numbering; typecheck/scoped lint/build OK.

## 🟡 PDF — purge yellow, increase section spacing, hide offer-id near footer logo

- [x] 2026-05-07 — Generated PDF-ში: (1) **ყვითელი backgrounds-ის სრული წაშლა** — `PDF_COLORS.yellow` removed from `colors.ts`, `highlightIndices` + `highlightValues` branches removed from `drawMetadataTable` + `drawBox` (Task 045-ის სამი ცვლილება უკან გადახდის). მონაცემი filled-ია, template hint აღარ საჭიროა. (2) **სექციებს შორის spacing გაიზრდება** — logo→title→provider→recipient→items→summary→features ყველა ცვლადი გავა breathing room-ით (specific values task-ში). (3) **`drawSmallHeader()`-ში offer.id top-right text მოხსნა** — overlap-ს ლოგოს `O-1816`-ის სტილში და უგვერდით 3-ე ფაგ. იქნება მხოლოდ logo, არცერთი text. **Codex**, NO DELETIONS, STRICTLY DMT offer PDF scope. Task: [docs/tasks/047-pdf-no-yellow-spacing-cleanup.md](./tasks/047-pdf-no-yellow-spacing-cleanup.md). Codex code ✅ 2026-05-07 — yellow color/highlight branches purged, PDF section spacing increased, continuation header no longer renders offer id top-right; grep/typecheck/lint/build OK. Browser PDF generation visual check blocked in this session by DMT API auth 401.

## 🟡 Invoices page — status + date filters (Phase 1 ✅ shipped)

- [x] 2026-05-07 — `/dmt/invoices` ცხრილში დავამატოთ ფილტრები: (1) **სტატუსით** multi-select pill-ები (draft/sent/approved/rejected/cancelled, "ყველა" + count badges, OR logic), (2) **თარიღით** range picker (from/to + 4 quick presets: ბოლო 7 / ბოლო 30 / ეს თვე / წინა თვე, ფილტრავს `updatedAt`-ზე). Filter state localStorage-ში persists (`dmt:invoices:filter`). Combined AND logic search-თან. "ფილტრების გასუფთავება" link active filter-ის დროს. **STRICTLY `/dmt/invoices` scope. NO DELETIONS.** Task: [docs/tasks/046-invoices-filters.md](./tasks/046-invoices-filters.md). Codex code ✅ 2026-05-07 — status OR pills, date range panel/presets, persisted filter state, active clear action, filtered empty state, and search AND logic shipped; typecheck/lint/build OK.

## 🟡 Invoices page — Phase 2: sort + sequential IDs

- [x] 2026-05-07 — `/dmt/invoices` ცხრილში დავამატოთ Phase 2 (Phase 1 already shipped above): (3) **სორტირება** clickable column headers — სტატუსი (predefined `STATUS_ORDER`: draft → sent → approved → rejected → cancelled) / ჯამი (numeric) / თარიღი (`updatedAt`). ↑/↓ indicator active key-ზე, click toggle dir, click new key → set + dir='desc'. (4) **ID column** → `№ {docNumber}` (1, 2, 3...) cryptic `o.id` hash-ის ნაცვლად. `o.docNumber` მოდის `dmt_next_offer_doc_number()` RPC-დან, populate-ხდება PDF გენერაციის დროს; თუ ჯერ null → `—` placeholder + tooltip-ში legacy id. Search-ი orient-ს ორ ფორმატზე (`№3` ან `r177...`). Sort state ემატება `dmt:invoices:filter` localStorage payload-ს. **`o.id` არ ვრიდიქტურდება DB-ში — routing key-ად რჩება, `docNumber` user-facing მხოლოდ.** Task: იხ. [docs/tasks/046-invoices-filters.md](./tasks/046-invoices-filters.md) sections H, I, J. Codex code ✅ 2026-05-07 — sortable status/total/date headers, persisted sort state, user-facing `№ docNumber` column, and doc-number search shipped; typecheck/scoped lint/build OK.
