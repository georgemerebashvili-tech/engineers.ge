...
- [x] 2026-04-19 — **Task 025** · Projects Gate + Tabs pattern (Claude localStorage API + UI ✅). [`tasks/025-project-gate.md`](./tasks/025-project-gate.md) · **Code ready**: `lib/projects.ts` + `components/projects/*` + `_project-bridge.js` wiring · 6 calcs integrated (wall-editor, stair, elevator, parking, floor, composer) · **Next: Codex** state persistence testing + Supabase link (Task 022)
...
- [ ] 2026-04-19 — **Legacy dark-mode support** · hvac.html, heat-loss.html, wall-thermal.html არ აქვთ `.dark` CSS variants (inline `:root` tokens only). სრული dark-mode-ისთვის სჭირდებათ `.dark` variant თითოეული color token-ისთვის. Scope: ~0.5 დღე. 🟢
...
- [x] 2026-04-19 — 🟡 **Calc iframes: language sync (interim fix)** · LangSwitch-ი `pathname.startsWith('/calc/')` route-ებზე hidden (`return null`). Promise broken გაქრა — calc-ებზე ენის ცვლა აღარ ჩანს, რადგან HTML-ები ქართულზე hardcoded-ია. *(Claude ✅ 2026-04-19)*
- [ ] 2026-04-19 — 🟡 **Calc iframes: real i18n solution** · 18 calc HTML-ის translation. სჭირდება decision: (ა) JS runtime i18n JSON (3–5 დღე), (ბ) locale-per-file (108 HTML, dup საფრთხე), (გ) React rewrite calc-ების (28+ დღე). მიზანი: როცა content-ი ინგლისურად მზადდება, ეს task-ი ცხოვრება.
